...
t_files_lst *ft_sort_ftime(t_env *e, long int time)
{
	...
	if (e->options[1][2] > '0')
	{
		while (tmp)
		{
			...
			(time == tmp->stat.st_mtimespec.tv_sec && ft_strcmp(e->tmp_name, tmp->f_name) > 0))
				return (tmp);
			tmp = tmp->next;
		}
	}
...
// 		while (tmp)
// 		{
// 			if (time < tmp->stat.st_mtimespec.tv_sec || (time == tmp->stat.st_mtimespec.tv_sec && ft_strcmp(name, tmp->f_name) > 0))
// 				return (tmp);
// 			tmp = tmp->next;
// 		}
// 	}
...
// 	if (e->options[1][3] == '0')
// 		tmp = ft_sort_fascii(e, file_name);
// 	else
// 		tmp = ft_sort_ftime(e, e->stat_tmp.st_mtimespec.tv_sec, file_name);
