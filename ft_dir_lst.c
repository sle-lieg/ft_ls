...
t_dir_lst *ft_sort_time(t_env *e, long int time)
{
	...
	if (e->options[1][2] > '0')
	{
		while (tmp)
		{
			if (time < tmp->d_time || (time == tmp->d_time && ft_strcmp(e->tmp_name, tmp->d_name) > 0))
				return (tmp);
			tmp = tmp->next;
		}
	}
...
{
	if (!(e->dir_lst))
		e->dir_lst = ft_add_dir(e, e->tmp_name, NULL, NULL);
	else
		ft_sort_dir(e);
	return (0);
}

// t_dir_lst	*ft_add_dir(t_env *e, char *path, char *d_name, t_dir_lst *pr, t_dir_lst *nx)
// {
// 	t_dir_lst *elem;
...
// t_dir_lst	*ft_sort_ascii(t_env *e, char *dir_name)
// {
// 	t_dir_lst	*tmp;
...
// 		while (tmp)
// 		{
// 			if (ft_strcmp(dir_name, tmp->d_name) > 0)
// 				return (tmp);
// 			tmp = tmp->next;
// 		}	
// 	}
...
// 		while (tmp)
// 		{
// 			if (time < tmp->d_time || (time == tmp->d_time && ft_strcmp(dir_name, tmp->d_name) > 0))
// 				return (tmp);
// 			tmp = tmp->next;
// 		}
// 	}
...
// 	if (tmp && !tmp->prev)
// 	{
// 		tmp->prev = ft_add_dir(e, dir_name, dir_name, NULL, tmp);
// 		e->dir_lst = tmp->prev;
// 	}
// 	else if (tmp)
// 	{
// 		tmp->prev->next = ft_add_dir(e, dir_name, dir_name, tmp->prev, tmp);
// 		tmp->prev = tmp->prev->next;
// 	}
// 	else
// 	{
...
// 		while (tmp->next)
// 			tmp = tmp->next;
// 		tmp->next = ft_add_dir(e, dir_name, dir_name, tmp, NULL);
// 	}
// }
...
// {
// 	if (!(e->dir_lst))
// 		e->dir_lst = ft_add_dir(e, dir_name, dir_name, NULL, NULL);
// 	else
